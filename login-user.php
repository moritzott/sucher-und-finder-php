    ...
    try {
        ...
        if($user) 
        {
            ...
            if(password_verify($password, $user["password"]))
            {
                ...
                // diese STelle noch überarbeiten:
                require_once "./settings/sessionsConfig.php";

                if($user["role"] === "orc")
                {
                    $_SESSION["role"] = $roles["normal-user"];
                }
                elseif ($user["role"] === "hobbit")
                {
                    $_SESSION["role"] = $roles["featured-user"];
                }
                elseif ($user["role"] === "aragorn")
                {
                    $_SESSION["role"] = $roles["admin"];
                }
                else
                {
                    // no role information
            ...
            else 
            {
                ...
                header("Location: /errors/not-logged-in.php"); // andere Seite dafür
                exit();
            }
